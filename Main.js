...
logger('∞∞Rahat Bot🥵 LOADED∞∞ Found appstate.json path successfully');
...
try {
  ...
  try {
    ...
    logger('Found file config: config.json', 'loader');
  } catch (err) {
    // try .temp
    if (existsSync(global.client.configPath.replace(/\.json/g, '') + '.temp')) {
      configValue = JSON.parse(readFileSync(global.client.configPath.replace(/\.json/g, '') + '.temp', 'utf8'));
      logger(`Found: ${global.client.configPath.replace(/\.json/g, '') + '.temp'}`, 'loader');
    } else {
...
try {
  ...
  logger('Found appstate.json path successfully', 'loader');
} catch (err) {
